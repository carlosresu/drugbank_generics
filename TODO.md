You are GPT-5.1-Codex-Max working inside this repo/workspace. Your task is to refactor the R script below so that its execution is genuinely multicore in practice.

Context:
- r-polars is multicore for Polars-native ops, but this script is not showing CPU usage because most heavy work happens in base R + R UDFs.
- Goal: move as much heavy computation as possible into Polars expressions / lazy plans, avoid `as.data.frame()` roundtrips, and replace R-side loops/UDFs with Polars-native operations where feasible.
- Keep output identical (same columns, same filters, same sorting, same files written).
- Do NOT introduce new repos or external services. Stay within R + r-polars + dbdataset already used here.

What “make it multicore” means here:
1. Identify the top single-thread bottlenecks:
   - base R `lapply/vapply/tapply` over large vectors
   - explicit `for` loops over rows (`collect_disallowed_pairs`, `expand_combos`, etc.)
   - `map_batches(function(x) { ...R code... })` that forces callbacks into R
   - any `as.data.frame()` conversions that pull data out of Polars early
2. Refactor those to Polars-native equivalents:
   - use Polars list/struct/string/regex expressions instead of R helpers where possible
   - use `with_columns`, `explode`, `group_by`, `agg`, `join`, `unique`, `sort`
   - prefer LazyFrames (`scan_*`) if it helps keep a single optimized plan
3. Ensure Polars thread settings apply:
   - move `Sys.setenv(POLARS_MAX_THREADS=...)` BEFORE `library(polars)`
   - don’t rely on single-thread R for the heavy stages
4. Keep correctness:
   - same row counts
   - same dedup logic
   - same NA/empty-string behavior
   - same ordering
   - same Parquet/CSV outputs

Deliverables:
A) Revised script (full file) with comments showing what changed and why.
B) A short section at top titled “Multicore strategy summary” listing:
   - which functions were moved into Polars
   - which (if any) must remain in R and why
   - expected impact on CPU usage
C) If some R-only logic truly can’t be expressed in r-polars today, then:
   - isolate it into the smallest possible R “island”
   - optionally parallelize that island using base R parallel tools ONLY if needed

Here is the script to refactor (treat as the full file):
...
- Do not drop columns.
- Do not change output file names/locations.
- Avoid adding new dependencies unless absolutely necessary and already common in base R.
- Prefer Polars solutions over adding R parallelism.

Start by writing a brief plan, then apply edits.